...
void dreptunghi(void)
{
	...
	glTranslated(200.0, i, 0.0); // modificat: miscarea se face pe axa oy, nu ox
	
	glPushMatrix(); // in stiva: 2 matrice, ambele T

	glRotated(-j, 0.0, 0.0, 1.0); // modificat: rotatia dreptunghiului rosu este inversata
...
{
	i = i + step;
	if (i > 550.0)           // modificat: limite pentru miscarea verticala
		step = -alpha;
	else if (i < 0.0)
		step = alpha;
	j = j + step;
...
void miscas(void)
{
	...
	if (i < 0.0)
		step = alpha;
	else if (i > 550.0)      // modificat: limita superioara
		step = -alpha;
	j = j + step;
	glutPostRedisplay();
}
...
void mouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:
		if (state == GLUT_DOWN)
			step = -alpha; glutIdleFunc(miscas);
		break;
	case GLUT_RIGHT_BUTTON:
		if (state == GLUT_DOWN)
			step = alpha; glutIdleFunc(miscad);
		break;
	default:
		break;
	}
